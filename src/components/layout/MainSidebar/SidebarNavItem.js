...
import './SidebarNavItem.css';
 ...
 class SidebarNavItem extends React.Component {
  ...
  render(){
    ...
    return(
  <NavItem  className="pointer nav-caret" tag={Dropdown} toggle={this.toggleUserActions} >
    {item.subMenu === "" ? 
    <li>
       ...
       {item.htmlBefore && (
        ...
        <Collapse open={visible} style={{ background: 'transparent'}}>
            ...
            return(
              <NavLink key={index} tag={RouteNavLink} to={menu.to} className="px-5 ">
                {menu.htmlBefore && (
                  <div
                    className="d-inline-block item-icon-wrapper"
                ...
                {menu.title && <span className="text-muted">{menu.title}</span>}
                {menu.htmlAfter && (
                  <div
                    className="d-inline-block item-icon-wrapper"
